...
def menu():
    ...
    print('(3) Anime by Season')
    print('(4) Search manga by name')
    print('(5) Top 10 manga')
...
def getTop(ranking_type , option , title):
    ...
    if select == 'x':
        ...
        print()
        os.system('pause')
        getTop(ranking_type , option , title)
...
def getAnimeBySeason():
    ...
    print(title)
    print('-'*len(title))
    print('(1) Winter (January, February, March)')
    print('(2) Spring (April, May, June)')
    print('(3) Summer (July, August, September)')
    print('(4) Fall (October, November, December)')
    print('(x) Main Menu')
    ...
    if season_data is None:
        ...
        print('Animes for ' + season.title() + ' ' + year)
    
        print()
        for idx, anime in enumerate(season_data):
            print(str(idx+1) + '. ' + anime['node']['title'])

        print()
        select = ''

        while (select.isdigit() == True and (int(select) > len(season_data) or int(select) < 1)) or (select.isdigit() == False and select != 'x'):
            select = input('Select to view (1-' + str(len(season_data)) +') or \'x\' to return to anime by season menu: ')
        ...
        if select == 'x':
            ...
            os.system('cls')
            detailResponse = getDetails(str(season_data[(int(select)-1)]['node']['id']), 'anime')
            displayDetails(detailResponse.json(), 'anime')
